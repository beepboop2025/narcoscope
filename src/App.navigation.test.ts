...
describe('App hash navigation', () => {
  it('resolves tab routes and newsroom deep links', () => {
    ...
    expect(resolveTabFromHash('#bri')).toBe('bri')
    ...
    expect(resolveTabFromHash('')).toBe('data')
    expect(resolveTabFromHash('#arms')).toBe('arms')
    expect(resolveTabFromHash('#economy')).toBe('economy')
