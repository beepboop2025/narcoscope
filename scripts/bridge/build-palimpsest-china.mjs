...
export async function loadBridgeInputs(root = defaultRoot) {
  ...
  const priceEdition = priceUrlMatch?.[0].match(/^https:\/\/www\.unodc\.org\/documents\/data-and-analysis\/WDR_(2025|2026)\/Annex\/8\.1_Prices_and_purities_of_drugs\.xlsx$/)?.[1]
  if (!generatedMatch || !priceUrlMatch || !priceEdition) {
...
function buildRetailPrices(inputs) {
  ...
  return {
    ...
    measurement: {
      ...
      method: 'UNODC typical retail value, with a midpoint fallback for a complete range; multiple reports and reviewed source forms are averaged at country, drug and year grain',
    ...
    provenance: {
      ...
      title: `World Drug Report ${inputs.prices.sourceEdition}, Statistical Annex 8.1: Prices and purities of drugs`,
      url: inputs.prices.sourceUrl,
      sourceEdition: inputs.prices.sourceEdition,
    ...
    limitations: [
      'Country and drug reporting is incomplete across years; absent rows are unavailable, and no missing year is interpolated.',
      'This legacy four-drug summary pools reviewed forms and reports; use the global drugs dataset for separate source entries and original units.',
      'WDR data reproduction is limited to educational and non-profit use with attribution; commercial reuse requires prior written UNODC permission.',
