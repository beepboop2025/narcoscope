...
function assertClaimSemanticsSourceTotal(semantics, readiness, label) {
  ...
  for (const [claimClass, count] of Object.entries(semantics.claimClassCounts)) {
    if (count > readiness.sourceCount) {
      throw new Error(`${label}.claimClassCounts.${claimClass} exceeds the source population`)
    }
  }
  const officialOrAdministrativeCount = (semantics.sourceClassCounts.legal ?? 0)
    + (semantics.sourceClassCounts.official_china ?? 0)
    + (semantics.sourceClassCounts.official_host ?? 0)
  if (semantics.officialOrAdministrativeSourceIds.length !== officialOrAdministrativeCount) {
    throw new Error(`${label}.officialOrAdministrativeSourceIds does not match derivable source-class counts`)
  }
  if (semantics.independentObservationSourceIds.length
    !== (semantics.authorityRoleCounts.independent_observation ?? 0)) {
    throw new Error(`${label}.independentObservationSourceIds does not match the independent-observation count`)
  }
  const modeledCount = semantics.claimClassCounts.modeled_estimate ?? 0
  const analyticalCount = semantics.claimClassCounts.analytical_estimate ?? 0
  const modeledOrAnalyticalCount = semantics.modeledOrAnalyticalSourceIds.length
  if (modeledOrAnalyticalCount < Math.max(modeledCount, analyticalCount)
    || modeledOrAnalyticalCount > Math.min(
      readiness.sourceCount,
      modeledCount + analyticalCount,
    )) {
    throw new Error(`${label}.modeledOrAnalyticalSourceIds is inconsistent with non-exclusive claim-class counts`)
  }
}
...
function assertTargetCoverage(value, label) {
  ...
  for (const [areaIndex, area] of value.entries()) {
    ...
    for (const [targetIndex, target] of area.targets.entries()) {
      ...
      target.sources.forEach((source, sourceIndex) => (
        assertSourceSummary(source, `${targetLabel}.sources[${sourceIndex}]`)
      ))
      ...
      target.sources.forEach((source) => {
        ...
        const prior = sourceMetadata.get(source.sourceId)
        if (prior !== undefined && prior !== serialized) {
          throw new Error(`${label} repeats sourceId ${source.sourceId} with inconsistent metadata`)
        }
        sourceMetadata.set(source.sourceId, serialized)
      })
...
function assertEconomicSnapshot(value, label) {
  ...
  let expectedCountrySeries = null
  for (const [countryIndex, country] of coverage.countries.entries()) {
    ...
    for (const [indicatorIndex, indicator] of country.indicators.entries()) {
      ...
      const descriptor = JSON.stringify({ indicatorId: indicator.indicatorId, unit: indicator.unit })
      const priorDescriptor = seriesMetadata.get(indicator.seriesId)
      if (priorDescriptor !== undefined && priorDescriptor !== descriptor) {
        throw new Error(`${indicatorLabel} metadata differs across countries for the same seriesId`)
      }
      seriesMetadata.set(indicator.seriesId, descriptor)
    ...
    const countrySeries = country.indicators.map((item) => item.seriesId)
    if (new Set(countrySeries).size !== countrySeries.length) {
      throw new Error(`${countryLabel}.indicators contains duplicate seriesId values`)
    }
    const sortedCountrySeries = [...countrySeries].sort()
    if (expectedCountrySeries === null) expectedCountrySeries = sortedCountrySeries
    else if (JSON.stringify(sortedCountrySeries) !== JSON.stringify(expectedCountrySeries)) {
      throw new Error(`${countryLabel}.indicators does not contain the exact shared series set`)
    }
  ...
  if (expectedCountrySeries?.length !== totals.indicators) {
    throw new Error(`${label}.coverage.totals.indicators does not match the shared country series count`)
  }
