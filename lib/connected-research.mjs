...
export const CONNECTED_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    data: { type: 'object', required: ['schema', 'regions', 'source_clocks', 'use_policy'],
      properties: { schema: { const: 'palimpsest.connected-research.v1' }, regions: { type: 'array', minItems: 5, maxItems: 5, items: { type: 'object' } }, source_clocks: { type: 'object' }, use_policy: { type: 'object' } }, additionalProperties: true },
...
export function validateConnectedResearch(data) {
  ...
  if (Object.keys(data.input_sha256 ?? {}).length !== 4) throw new Error('Research inputs are not pinned')
  if (Object.keys(data.input_sha256).sort().join(',') !== 'china,economy,partner,wire') throw new Error('Research input identities changed')
